  ...
  return (
    <header className="sticky top-0 z-50 border-b border-sky-mid/80 bg-white/85 backdrop-blur-xl">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-2.5 sm:px-6">
        ...
        <nav className="hidden items-center gap-6 lg:flex" aria-label="หลัก">
          {links.map((link) => (
        ...
        <button
          type="button"
          className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-sky-mid text-royal md:hidden"
          aria-expanded={open}
      ...
      {open ? (
        <div id="mobile-nav" className="border-t border-sky-mid bg-white px-4 py-4 md:hidden">
          <nav className="grid gap-1" aria-label="มือถือ">
            {links.map((link) => (
              <a
                key={link.href}
                href={link.href}
